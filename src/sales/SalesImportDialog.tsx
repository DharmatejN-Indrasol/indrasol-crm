...
import { SalesImportSchema, useSalesImport } from './useSalesImport';
import { MouseEvent, useEffect, useState } from 'react';
// TODO: Replace with actual sample CSV for sales
const SAMPLE_URL = '';

type SalesImportModalProps = {
    open: boolean;
    onClose(): void;
};
...
export function SalesImportDialog({
    ...
    const processBatch = useSalesImport();
    const { importer, parseCsv, reset } = usePapaParse<SalesImportSchema>({
        batchSize: 10,
        processBatch,
    });
    ...
    return (
        <Dialog open={open} maxWidth="md" fullWidth aria-labelledby="sales-import-dialog-title">
            <DialogCloseButton onClose={handleClose} />
            <DialogTitle id="sales-import-dialog-title">Import</DialogTitle>
                ...
                <Form>
                    <Stack spacing={2}>
                        {importer.state === 'running' && (
                            <Stack gap={2}>
                                ...
                                <Typography variant="body2">
                                    Imported <strong>{importer.importCount} / {importer.rowCount}</strong> users, with <strong>{importer.errorCount}</strong> errors.
                        ...
                        {importer.state === 'complete' && (
                            <Alert severity="success">Users import complete. Imported {importer.importCount} users, with {importer.errorCount} errors</Alert>
                        )}
                        {importer.state === 'idle' && (
                            <>
                                <Alert
