...
import { Contact, EmailAndType } from '../../types';
...
it('should return gravatar URL for anthony@Indrasollab.com', async () => {
    const email: EmailAndType[] = [
        { email: 'anthony@Indrasollab.com', type: 'Work' },
    ];
...
it('should return null if email has no gravatar or validate domain', async () => {
    const email: EmailAndType[] = [
        { email: 'anthony@fake-domain-Indrasollab.com', type: 'Work' },
    ];
    const record: Partial<Contact> = { email_jsonb: email };
