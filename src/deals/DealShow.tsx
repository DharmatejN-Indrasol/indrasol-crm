...
const DealShowContent = ({ handleClose }: { handleClose: () => void }) => {
    ...
    const [ai, setAI] = useState<{status?: string, nextAction?: string, forecast?: string, error?: string}>({});
    ...
    useEffect(() => {
        let cancelled = false;
        setAILoading(true);
        // TODO: Replace with getAIStatus/getAIForecast when available
        const aiStatus = Number(record.id) % 2 === 0 ? 'AI: High chance to close' : 'AI: At risk';
        getAINextAction({ type: 'deal', record }).then(res => {
            if (!cancelled) {
                setAI({
                    status: aiStatus,
                    nextAction: res.action,
                    forecast: 'AI: Expected revenue this quarter: $12,000.',
                    error: res.error,
                });
                setAILoading(false);
            }
