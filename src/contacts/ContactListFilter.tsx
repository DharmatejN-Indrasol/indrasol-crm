...
import { Box, Chip, IconButton, Tooltip, Divider, Collapse, Stack, Typography } from '@mui/material';
...
const SIDEBAR_COLLAPSED_KEY = 'contact_sidebar_collapsed';
...
const ContactListFilter = () => {
    ...
    const owners = React.useMemo(() => Array.from(new Set((contacts || []).map(l => l.owner_id).filter(Boolean))), [contacts]);
    const sources = React.useMemo(() => Array.from(new Set((contacts || []).map(l => l.source).filter(Boolean))), [contacts]);
